...
class NavigationManager implements INavigationManager {
	...
	/** Top-level app menu allowlist for this private-drive fork. */
	private const DRIVE_LINK_ALLOWLIST = ['files', 'gallery', 'settings'];
	...
	public function getAll(string $type = 'link'): array {
		...
		// Private-drive product: only Files, Gallery, and Settings in the apps menu.
		if ($type === 'link' || $type === 'all') {
			$result = array_filter($result, function ($entry) use ($type) {
				...
				$id = $entry['id'] ?? '';
				$app = $entry['app'] ?? $id;
				return in_array($id, self::DRIVE_LINK_ALLOWLIST, true)
					|| in_array($app, self::DRIVE_LINK_ALLOWLIST, true);
			});
		}
	...
	public function getDefaultEntryIds(bool $withFallbacks = true): array {
		$this->resolveAppNavigationEntries();
		$storedIds = explode(',', $this->config->getSystemValueString('defaultapp', $withFallbacks ? 'files' : ''));
		$ids = [];
		$entryIds = array_keys($this->entries);
